...
start() {
  ...
  if [ $? -eq "1" ]; then
    ...
    # jar服务启动脚本
    mkdir -p $LOG_FILE/gc
    nohup java $JAVA_OPTS -Xloggc:$LOG_FILE/gc/gclog.log -XX:HeapDumpPath=$LOG_FILE/gc/HeapDump.hprof -jar $JAR_NAME --server.port=$SERVER_PORT --spring.profiles.active=$SERVER_ENV   >./$LOG_FILE/run.log 2>&1 &
    echo $! > $JAR_PID
